...
func All() []Model {
	return []Model{
		...
		},
		{
			...
			Factory: func(config *Config, httpc *http.Client) (llm.Service, error) {
				if config.AnthropicAPIKey == "" {
					return nil, fmt.Errorf("claude-fable-5 requires ANTHROPIC_API_KEY")
				}
				svc := &ant.Service{APIKey: config.AnthropicAPIKey, Model: ant.Fable5, HTTPC: httpc, ThinkingLevel: llm.ThinkingLevelMedium}
				if url := config.getAnthropicURL(); url != "" {
					svc.URL = url
				}
				return svc, nil
		...
		},
		{
			...
			Factory: func(config *Config, httpc *http.Client) (llm.Service, error) {
				if config.FireworksAPIKey == "" {
					return nil, fmt.Errorf("glm-5.2-fireworks requires FIREWORKS_API_KEY")
				}
				svc := &oai.Service{Model: oai.GLM52Fireworks, APIKey: config.FireworksAPIKey, HTTPC: httpc}
...
func (m *Manager) createServiceFromModel(model *generated.Model) llm.Service {
	thinkingLevel := llm.ParseThinkingLevel(model.ThinkingLevel)
